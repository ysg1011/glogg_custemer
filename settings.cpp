...
void Settings::settingChanged(Settings::Options option, QVariant setting)
{
    ...
    case LineTermination:
        session.lineEnd = static_cast<Settings::LineTerminator>(setting.toInt());
        break;
    case CharacterDelay:
...
void Settings::readSessionSettings(QSettings &settings)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        session.openMode = (readUIntSetting(settings, QStringLiteral("OpenMode"), &value))
                               ...
                               : QIODevice::ReadWrite;
        session.lineEnd = (readUIntSetting(settings, QStringLiteral("LineEnd"), &value))
                              ? static_cast<Settings::LineTerminator>(value)
                              : Settings::LF;
...
void Settings::saveSessionSettings()
{
    ...
    if (m_sessions.size() > 0) {
        ...
        while (iter.hasNext()) {
            ...
            settings.setValue("LineEnd", session.lineEnd);
