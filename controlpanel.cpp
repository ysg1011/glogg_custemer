...
void ControlPanel::closeDeviceExtern()
{
    toggleDeviceExtern(false, false);
    m_bt_open->setChecked(false);
}

void ControlPanel::toggleDevice(bool open) { toggleDeviceExtern(open, true); }

void ControlPanel::toggleDeviceExtern(bool open, bool emitSignal)
{
    if (open) {
        if (m_menuVisible)
            toggleMenu();
