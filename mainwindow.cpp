    ...
    , m_reconnectTimer(this)
{
    ...
    fillLineTerminationChooser();
    applyLineTermination(m_settings->getCurrentSession().lineEnd);
    connect(m_combo_lineterm, &QComboBox::currentIndexChanged, [=]() {
        auto lt = m_combo_lineterm->currentData().value<Settings::LineTerminator>();
        m_settings->settingChanged(Settings::LineTermination, lt);
        applyLineTermination(lt);
    });
    ...
    connect(m_device, &QSerialPort::errorOccurred, this, &MainWindow::handleError);
    ...
    connect(actionFont, &QAction::triggered, this, &MainWindow::setupFont);
...
void MainWindow::openDevice()
{
    ...
    } else if (m_deviceState != DEVICE_RECONNECT) {
        controlPanel->closeDeviceExtern();
        handleError(m_device->error());
    }
}

QString MainWindow::serialPortErrorToString(QSerialPort::SerialPortError error)
{
    switch (error) {
    ...
    case QSerialPort::SerialPortError::NotOpenError:
        return tr("Not Open Error");
    };
    return "Unhandled Serialport Error";
}

void MainWindow::setDeviceState(DeviceState state)
{
    m_deviceState = state;
    m_device_statusbar->setStatus(deviceStateToString(m_deviceState));
}

QString MainWindow::deviceStateToString(DeviceState state)
{
    switch (state) {
    case DeviceState::DEVICE_CLOSED:
        return tr("Device Closed");
    case DeviceState::DEVICE_OPENING:
        return tr("Device Opening");
    case DeviceState::DEVICE_OPEN:
        return tr("Device Open");
    case DeviceState::DEVICE_CLOSING:
        return tr("Device Closing");
    case DeviceState::DEVICE_RECONNECT:
        return tr("Device Reconnect");
    }
...
void MainWindow::handleError(QSerialPort::SerialPortError error)
{
    if (error == QSerialPort::NoError) {
        ...
        if (m_settings->getCurrentSession().autoReconnect) {
            setDeviceState(DEVICE_RECONNECT);
            disableInput();
            ...
            msgBox.setText(tr("Error: %1").arg(serialPortErrorToString(m_device->error())));
            msgBox.setInformativeText(
                tr("<a href=\"%1\">Get Help!</a>").arg("https://gitlab.com/cutecom/cutecom#faq/"));
            msgBox.exec();
...
{
    QFont new_font = QFontDialog::getFont(0, this->m_command_history->font());
    this->m_command_history->setFont(new_font);
    this->m_output_display->setupTextFormats(&new_font);
}
...
void MainWindow::fillLineTerminationChooser()
{
...
void MainWindow::applyLineTermination(Settings::LineTerminator lt)
{
    // Update the line termination chooser combobox
    if (lt != m_combo_lineterm->currentData()) {
        int index = m_combo_lineterm->findData(QVariant::fromValue((lt > Settings::HEX) ? Settings::LF : lt));
        if (index != -1) {
    ...
    m_ctrlCharactersPopup->setHexInsertionMode(Settings::LineTerminator::HEX == lt);
}
...
bool MainWindow::sendString(QString *s)
{
    ...
    {
        QString hex = *s;
        hex.remove(QRegularExpression("\\s+(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)")); // spaces except that in quotes
...
void MainWindow::sendFile()
{
    ...
    if (protocol == Settings::PLAIN) {
        ...
        qsizetype step = data.size() / 100;
        if (step < 1) {
            step = 1;
        }
        for (qsizetype i = 0; i < data.size(); i++) {
            if ((i % step) == 0) {
                m_progress->setValue(static_cast<int>(i / step));
                qApp->processEvents();
            }
            ...
            if (!rc) {
                QMessageBox::information(this, tr("Comm error"), tr("Sending failed (%1/%2").arg(i).arg(data.size()));
                break;
            }
...
void MainWindow::readFromStdErr()
{
    ...
    QString s(ba);
    QRegularExpression regex(".+\\d+/ *(\\d+)k.*");
